...
class Command(BaseCommand):
    help = (
        "Download, normalize, and import Roadmap segmentations into per-state "
        "GenomicFeatures grouped by Collection"
    )
    ...
    def _get_reference_genome(name: str) -> ReferenceGenome:
        try:
            ...
            raise CommandError(f"ReferenceGenome {name} not found. Import genomes first.")

    def _download_segmentation(self, url: str, tmpdir: str) -> str:
        self.stdout.write(f"Downloading {url} ...")
        path = _download_file(url, tmpdir)
        return str(path)
    ...
    def _get_or_create_collection(
        ...
        collection, _ = GenomicFeatureCollection.objects.get_or_create(
            ...
            defaults={
                "description": (
                    f"Roadmap {cfg['reference']} segmentations for {cell_name}"
                ),
    ...
    def _write_temp_bed(
        ...
        safe_state = re.sub(r"[^A-Za-z0-9._-]", "_", state_name)
        bed_file = os.path.join(tmpdir, f"{eid}_{safe_state}.bed")
    ...
    def _sort_and_tabix(
        ...
        feature_label = f"{eid} - {state_name}"
        self.stdout.write(f"Sorting + tabix {feature_label} ...")

        sorted_bt = BedTool(bed_file).sort(header=True)
        tabix_bt = sorted_bt.tabix(force=True, is_sorted=True)

        safe_state = re.sub(r"[^A-Za-z0-9._-]", "_", state_name)
        bed_gz = os.path.join(tmpdir, f"{eid}_{safe_state}.bed.gz")
        bed_tbi = bed_gz + ".tbi"
    ...
    @staticmethod
    def _split_by_state(gz_path: str, state_map: dict) -> dict:
        """
        Read a chromHMM segmentation .bed.gz file and group entries by state
        using the provided state_map (state_number -> state_name).
        """
        ...
        with gzip.open(gz_path, "rt") as infile:
            for line in infile:
                ...
                state_name = state_map[parts[3]]

                state_records[state_name].append(
                    "\t".join([chrom, start, end, state_name])
                )
    ...
    def _process_state(
        ...
        feature_name = f"{cell_name} - {state_name}"
        ...
        _validate_against_chrom_sizes(
            bed_file=bed_file,
            chrom_size_file=reference_genome.chrom_size_file_bed.path,
        )
    ...
    def _get_or_create_feature(
        ...
        try:
            ...
            return GenomicFeature(
                name=feature_name,
                description=(
                    f"{cfg['reference']} {state_name} segmentation "
                    f"for {cell_name}"
                ),
                reference_genome=reference_genome,
                collection=collection,
    ...
    def _process_model_for_cell(
        ...
        url = cfg["base_url"] + cfg["fname_template"].format(eid=eid)

        try:
            gz_path = self._download_segmentation(url, tmpdir)
        ...
        state_records = self._split_by_state(gz_path, cfg["state_map"])
        if not state_records:
            self.stdout.write(
                self.style.WARNING(
                    f"No usable records for {eid} ({cell_name}) {model_key}, skipping."
                )
            )
            return
        ...
        for state_name, lines in state_records.items():
            self._process_state(
                eid=eid,
                cell_name=cell_name,
                state_name=state_name,
    ...
    def handle(self, *args, **options):
        force = options["force"]
        reference_genome = self._get_reference_genome("hg38")

        for eid, cell_name in CELL_TYPE_MAP.items():
            for model_key, cfg in MODEL_CONFIGS.items():
                with tempfile.TemporaryDirectory() as tmpdir:
                    self._process_model_for_cell(
                        eid=eid,
                        cell_name=cell_name,
                        model_key=model_key,
                        cfg=cfg,
