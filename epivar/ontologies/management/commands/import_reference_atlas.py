...
from ontologies.models import AnatomicalStructure, CellType # noqa

from ._private import _download_file, _parse_asct_table
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for table_url in tables:
            path = _download_file(table_url)
            asct_table = _parse_asct_table(path)

            for _, row in tqdm(asct_table.iterrows(), desc=table_url.split("/")[-1]):
                ...
                except ValidationError:  # to avoid ValidationError when signal cannot find ontology
                    continue             # e.g.  ['Term CL:4030010  not found.']
